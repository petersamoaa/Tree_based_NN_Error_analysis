...
from .gcnn import GCNNM
from .layers import Embedding, LayerNorm, PositionalEmbedding, RightTransformerBlock, DecodeTransformerBlock
...
class TreeAttEncoder(nn.Module):
    ...
    def forward(self, input_code, input_codechar, inputAd):
        ...
        char_em = self.char_embedding(input_codechar)
        char_em = self.conv(char_em.permute(0, 3, 1, 2))
        char_em = char_em.permute(0, 2, 3, 1).squeeze(dim=-2)
        x = self.token_embedding(input_code.long())
        for trans in self.transformerBlocksTree:
            x = trans.forward(x, codemask, char_em, inputAd, True)
        for trans in self.transformerBlocks:
            x = trans.forward(x, codemask, char_em)
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self, inputnl, inputnlchar, inputrule, inputruleparent, inputrulechild, inputParent, inputParentPath, inputdepth, tmpf, tmpc, tmpindex, rulead, antimask,
                inputRes=None, mode="train"):
        selfmask = antimask
